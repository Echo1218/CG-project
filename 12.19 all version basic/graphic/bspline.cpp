...
void Bspline::drawcurve(QPixmap &tempix, QPixmap &pix, QPen &pen)
{
    ...
    for(double u=3;u<ctrlnum;u+=0.001)
    {
        double tmpx=0,tmpy=0;
        for(int i=0;i<ctrlnum;i++)
        {
            deltax=ctrlpoints[2*i];deltay=ctrlpoints[2*i+1];
            deltax*=B(i,4,u);deltay*=B(i,4,u);
    ...
    for(int j=0;j<curvepoints.size()/2-1;j++)
        bre.breline(tempix,tempix,pen,curvepoints[j*2],curvepoints[j*2+1],curvepoints[j*2+2],curvepoints[j*2+3]);
