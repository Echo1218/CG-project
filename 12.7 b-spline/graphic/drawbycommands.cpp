...
void DrawByCommands::cmdresolver(QString cmd)
{
    ...
    else if(list[0]=="translate")
    {
        ...
        for(int i=0;i<graphs.size();i++)
        {
            ...
            else if(graphs[i]->type==BSPLINECUR)
            {
                ((Bspline*)graphs[i])->drawcurve(pix,pix,temppen);
            }
        }
    ...
    else if(list[0]=="drawCurve")
    {
        if(list[3]=="Bezier")
        {
            ...
            bspline->id=list[1].toInt();
            bspline->type=BSPLINECUR;
            bspline->ctrlnum=list[2].toInt();
            bspline->color=color;
            graphindex.push_back(bspline->id);
            QString t;
            if(!file.atEnd())
            {
               t=file.readLine();
            ...
            for(int i=0;i<list.length();i++)
            {
                bspline->ctrlpoints.push_back(list[i].toInt());
            }
            bspline->drawcurve(pix,pix,pen);
            graphs.push_back(bspline);
    ...
    else if(list[0]=="clip")
    {
        ...
        if(list[6]=="Cohen-Sutherland")
        {
            if(graphs[index]->type==DDALINE)
            {
                ...
                ((Bresenham*)graphs[index])->C_Sclip(list[2].toInt(),list[3].toInt(),list[4].toInt(),list[5].toInt());
            }
        }
        else{
            if(graphs[index]->type==DDALINE)
            {
                ((DDA*)graphs[index])->L_Bclip(list[2].toInt(),list[3].toInt(),list[4].toInt(),list[5].toInt());
            }
            else{
                ((Bresenham*)graphs[index])->L_Bclip(list[2].toInt(),list[3].toInt(),list[4].toInt(),list[5].toInt());
            }
        }
    ...
    else if(list[0]=="rotate")
    {
        ...
        pix=QPixmap(canvasx,canvasy);
        pix.fill(Qt::white);
        graphs[index]->rotate(list[2].toInt(),list[3].toInt(),list[4].toInt());
    ...
    else if(list[0]=="scale")
    {
        ...
        pix=QPixmap(canvasx,canvasy);
        pix.fill(Qt::white);
        graphs[index]->scale(list[2].toInt(),list[3].toInt(),list[4].toFloat());
